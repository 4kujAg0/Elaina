...
exports.cmdAdd = function(run, time, _db){
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./ACTION/STORAGE/X-DATA/hitToday.json', JSON.stringify(_db))
    } else {
        const bulin = ({
        	id: run,
...
fs.writeFileSync('./ACTION/STORAGE/X-DATA/hitToday.json', JSON.stringify(_dir))
...
_level.push(obj)
fs.writeFileSync('./ACTION/STORAGE/X-DATA/hitToday.json', JSON.stringify(_level))
}
...
_level.push(obj)
fs.writeFileSync('./ACTION/STORAGE/X-DATA/claim.json', JSON.stringify(_level))
}
...
exports.expiredClaim = ( _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            _dir.splice(position, 1)
            fs.writeFileSync('./ACTION/STORAGE/X-DATA/claim.json', JSON.stringify(_dir))
        }
    }, 2000)
}
...
exports.createDataId= (nama,_level) => {                                                                                                                      
 obj = { name: nama, id: [] }
_level.push(obj)
fs.writeFileSync('./ACTION/STORAGE/X-DATA/data.json', JSON.stringify(_level))
}
...
exports.addDataId = function(idgc, nama, _db){
    ...
    if (found !== false) {
        ...
        fs.writeFileSync('./ACTION/STORAGE/X-DATA/data.json',JSON.stringify(_db));
    }
}
...
exports.Succes = function(cmd, _db){
    ...
    if (position !== false) {
        ...
        fs.writeFileSync('./ACTION/STORAGE/X-DATA/dashboard.json', JSON.stringify(_db))
    } else {
        const bulin = ({
            cmd: cmd,
            succes: 1,
