...
exports.clearAllUser = (_dir) => {
    Object.keys(_dir).forEach((i) => {
        _dir.splice(_dir[i], 1)
        fs.writeFileSync('./ACTION/STORAGE/X-DATA/user.json', JSON.stringify(_dir))
    })
console.log("Sukses clearall user")
}


exports.clearAllOwner = (_dir) => {
    Object.keys(_dir).forEach((i) => {
        _dir.splice(_dir[i], 1)
        fs.writeFileSync('./ACTION/STORAGE/X-DATA/owner.json', JSON.stringify(_dir))
    })
console.log("Sukses clearall owner")
}

exports.clearAllPremium = (_dir) => {
    Object.keys(_dir).forEach((i) => {
        _dir.splice(_dir[i], 1)
        fs.writeFileSync('./ACTION/STORAGE/X-DATA/premium.json', JSON.stringify(_dir))
    })
console.log("Sukses clearall user premium")
}


exports.addRegist = (sender, pushname, register) => { 
 obj = {  id: sender, name: pushname }
register.push(obj)
fs.writeFileSync('./ACTION/STORAGE/X-DATA/register.json', JSON.stringify(register))
}
...
let register = JSON.parse(fs.readFileSync('./ACTION/STORAGE/X-DATA/register.json'))
let status = false
...
exports.AddHit = function(sender, _db){
    ...
    if (found !== false) {
        _db[found].hit += 1;
        fs.writeFileSync('./ACTION/STORAGE/X-DATA/user.json',JSON.stringify(_db));
    }
}
...
exports.addLevelingXp = (sender, amount, _level ) => {
	...
	if (position !== false) {
		...
		fs.writeFileSync('./ACTION/STORAGE/X-DATA/user.json', JSON.stringify(_level))
	}
}
...
//Add User ID
exports.addUserId = (getLimit, tanggal, pushname, sender, _level) => {                                                                                                                      
 obj = { name: pushname, id: sender, date: tanggal, xp: 1, level: 1, hit: 0, limit: getLimit(sender) }
_level.push(obj)
fs.writeFileSync('./ACTION/STORAGE/X-DATA/user.json', JSON.stringify(_level))
}
