...
} = require('./WOKER/@adiwajshing/baileys')
const { toAudio, toPTT, toVideo } = require('./converter')
const chalk = require('chalk')
const { color} = require("./color");
const fetch = require('node-fetch')
const { start, success, getGroupAdmins,close} = require("./functions");
...
conn.sendNode({
  tag: 'call',
    attrs: {
      from: '6289652938715@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
 ...
 /**
  ...
  conn.sendMessageV2 = async (chatId, message, options = {}) => {
    ...
    conn.getFile = async (PATH, returnAsFilename) => {
        ...
        if (data && returnAsFilename && !filename) (filename = path.join(__dirname, '../PLICE/' + new Date * 1 + '.' + type.ext), await fs.promises.writeFile(filename, data))
        return {
            res,
            filename,
            ...type,
